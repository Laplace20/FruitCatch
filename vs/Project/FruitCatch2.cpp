...
Engine::FruitCatch2::FruitCatch2()
{
}
...
void Engine::FruitCatch2::Init()
{
	...
	// Background
	wallpp = new Texture("Sansw.png");
	wallppSprite = new Sprite(wallpp, game->GetDefaultSpriteShader(), game->GetDefaultQuad());
	wallppSprite->SetScale(1)
	...
	FC = new Text("Cyberpunks Italic.ttf", 100, game->GetDefaultTextShader());
	FC->SetPosition(game->GetSettings()->screenWidth - 850.0f, game->GetSettings()->screenHeight - 380.0f)->SetColor(18, 18, 74)->SetScale(0.81f);

	//Inisiasi Music n SFX
	megaSong = (new Music("Megalovania.ogg"))->SetVolume(70);
...
void Engine::FruitCatch2::Update()
{
	game->selectedScreen = 2;
	...
	if (gstate == GameState::RUNNING) {
		...
		if (duration / 1000 >= 0.62f) {
			if (megaSong->IsPlaying() == false)
			{
				megaSong->Play(false);
				megaSong->IsPlaying() == true;
		...
		//Score
		scoreText->SetText("Score: " + std::to_string(game->scoreMegalovania));
		scoreText2->SetText("Score: " + std::to_string(game->scoreMegalovania));


		//1 beat = 1 / 2.6 of a second
		bps = (duration / 1000) * 4.0f;
		...
		else if (duration / 1000 > 16 && duration / 1000 < 82)
		{
			if (floor(bps) > previousBps)
			{
				previousBps = floor(bps);  // Update the previous BPS value
		...
		else if (duration / 1000 > 82 && duration / 1000 < 112)
		{
			if (floor(bps) > previousBps)
			{
				previousBps = floor(bps);  // Update the previous BPS value
		...
		else if (duration / 1000 > 113 && duration / 1000 < 145)
		{
			if (floor(bps) > previousBps)
			{
				previousBps = floor(bps);  // Update the previous BPS value
		...
		else if (duration / 1000 > 146 && duration / 1000 < 155)
		{
			if (floor(bps) > previousBps)
			{
				previousBps = floor(bps);  // Update the previous BPS value
		...
		totalScore->SetText("Your Score : " + std::to_string(game->scoreMegalovania));
...
void Engine::FruitCatch2::ResetVariable() {
...
void Engine::FruitCatch2::FruitsClear() {

}

void Engine::FruitCatch2::UpdateBoundingBoxes() {
...
void Engine::FruitCatch2::SpawnFruit() {
...
void Engine::FruitCatch2::CheckCollision() {
	for (size_t i = 0; i < fruits.size(); ++i) {
		...
		if (pos.x < leftBox.x + leftBox.width && pos.x + fruitWidth > leftBox.x &&
			...
			int goodPoint = 50;
			game->scoreMegalovania += goodPoint;
			...
			game->scoreMegalovania += perfectPoint;
